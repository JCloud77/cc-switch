...
pub(crate) fn build_effective_settings_with_common_config(
    ...
    // 池是唯一真相：先按中央共享 Key 池 hydrate 一份内存副本，后续共用配置判定、
    // OAuth 判定、默认值处理与 Key 物化都必须基于这份副本。传入的 Provider 可能来自
    // provider add/update 请求（只有 `selectedKeyId`、没有 `apiKeys`），其 API Key 列表
    // 在 DAO 读取时才会 hydrate；这里不写回数据库。
    let mut hydrated = provider.clone();
    db.hydrate_shared_keys_for_provider_in_db(app_type.as_str(), &mut hydrated)?;
    ...
    // 单点物化：官方的 config-only 写入与写入预检闸只读 `settings_config.auth`，
    // 不认识中央池；在此把池中当前选中的 Key 写进内存 effective settings，保证这些
    // 链路看到的是真实生效凭据。数据库不动。
    super::shared_key_live::materialize_selected_shared_key(
        app_type,
        provider,
